...
describe('transactionOfSet', () => {
  it('set succeeded', async (done) => {
    ...
    const result = await transactionOfSet({
      ref: new MockDocumentReference(Promise.resolve(new MockDocumentSnapshot(false, { name: 'test', count: 1 }))),
      data: { name: 'test' },
      merge: true,
      transaction
    })
  ...
  it('error occured', async (done) => {
    ...
    const result = await transactionOfSet({
      ref: new MockDocumentReference(Promise.resolve(new MockDocumentSnapshot(true, { name: 'test', count: 1 }))),
      data: { name: 'test' },
      merge: true,
      transaction,
      errorHandler
