...
class FirestoreSubscriber {
    ...
    subscribe(state, commit, options) {
        ...
        if (!state[configurations_1.FIREX_UNSUBSCRIBERS]) {
            state[configurations_1.FIREX_UNSUBSCRIBERS] = new Map();
        }
        if (state[configurations_1.FIREX_UNSUBSCRIBERS].has(this.statePropName)) {
            return;
        }
